            ...
            Future future = service.submit(new Callable<Object>() {
                public Object call() {
                    try {
                        ...
                        sendMsg(History.loadHistory(getId()));
    ...
    private void autorization() throws IOException {
        while (true) {
            ...
            if (str.startsWith("/auth")) {
                ...
                id = authService.getId(tokens[1], tokens[2]);
                if (newNick != null) {
                    nick = newNick;
                    sendMsg("/authOK");
    ...
    void sendMsg(String message) {
        try {
            ...
            if(!message.startsWith("/")) {
                FileWriter historyWriter = new FileWriter("history/" + id + "_" + "history.txt", true);
                BufferedWriter bufferWriter = new BufferedWriter(historyWriter);
                bufferWriter.write(System.lineSeparator() + message);
                bufferWriter.close();
            }
        } catch (IOException e) {
            e.printStackTrace();
